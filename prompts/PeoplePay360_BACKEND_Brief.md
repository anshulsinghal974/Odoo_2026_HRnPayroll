...
- **F1:** "Scaffold `backend/` with Express + TypeScript, Prisma, a health check route (`GET /health`), and env config loading via `.env`. No business modules yet."
  → commit: `[BE-01.1] Express + Prisma scaffold`

### BE-02 · Schema, Migrations & Contract
- **F1:** "In `prisma/schema.prisma`, model all entities from the PRD's data model section: Employee, Contract, WorkingSchedule, ScheduleLine, TimeOffType, Allocation, LeaveRequest, Attendance, SalaryStructure, SalaryRule, Payrun, Payslip, PayslipLine, User (with Role enum). Add the constraint: only one ACTIVE contract per employee (unique partial index or app-level check documented in a comment). Run the initial migration."
  → commit: `[BE-02.1] Full Prisma schema + migration`
- **F2:** "Write a first-pass OpenAPI spec in `shared/api-contracts/openapi.yaml` covering all planned REST endpoints (paths, methods, request/response shapes) for every module below, even before they're implemented."
  → commit: `[BE-02.2] OpenAPI contract draft`

### BE-03 · Auth & RBAC Middleware
- **F1:** "Build `modules/auth/`: register/login endpoints, bcrypt password hashing, JWT issuance with role claim."
  → commit: `[BE-03.1] Auth endpoints`
- **F2:** "Build `middleware/jwt.middleware.ts` and `middleware/rbac.middleware.ts` (role-based route guards), and a seed script that creates 5 test users, one per role."
  → commit: `[BE-03.2] RBAC middleware + role seed`

### BE-04 · Employee & Contract APIs
- **F1:** "Build full CRUD for Employee in `modules/employees/`, plus a smart-count aggregate endpoint returning related contract/attendance/leave counts."
  → commit: `[BE-04.1] Employee CRUD + smart counts`
- **F2:** "Build Contract CRUD in `modules/contracts/` with the no-concurrent-active-contract rule enforced, and a period-resolution helper `resolveContractForPeriod(employeeId, date)` used later by the computation engine."
  → commit: `[BE-04.2] Contract CRUD + period resolution`

### BE-05 · Schedules & Time Off APIs
- **F1:** "Build Working Schedule + ScheduleLine CRUD with auto-calculated weekly hours on save."
  → commit: `[BE-05.1] Schedule APIs`
- **F2:** "Build TimeOffType, Allocation, and LeaveRequest CRUD with the approval flow and auto-balance-deduction logic on approval."
  → commit: `[BE-05.2] Time off + allocation + approval flow`

### BE-06 · Attendance APIs
- **F1:** "Build Attendance CRUD with worked-hours computation from check-in/out and schedule break time, plus status derivation (On Time/Late/Absent/Overtime/Missing Check-out)."
  → commit: `[BE-06.1] Attendance CRUD + status derivation`
- **F2:** "Add a manual correction endpoint that stores original value + corrector + timestamp for audit."
  → commit: `[BE-06.2] Attendance correction + audit`

### BE-07 · Salary Engine
- **F1:** "Build SalaryStructure + SalaryRule CRUD, enforcing unique sequence within a structure and supporting FIXED/PERCENTAGE/FORMULA computation types."
  → commit: `[BE-07.1] Salary structure + rule CRUD`
- **F2:** "Build the core computation engine: given an employee + period, resolve contract → fetch ordered rules → execute each in sequence (formula rules can reference prior rule codes as variables) → return computed PayslipLines + totals. Add unit tests for at least Basic → HRA% → Gross → PF% → Net."
  → commit: `[BE-07.2] Salary computation engine + tests`

### BE-08 · Payrun & Payslip APIs
- **F1:** "Build the 2-step Payrun creation endpoint (validate scope, then create with selected employees) and the Compute endpoint that runs the computation engine per employee and persists Payslips + PayslipLines."
  → commit: `[BE-08.1] Payrun creation + compute`
- **F2:** "Build Validate, Mark-Paid, and warning-detection logic (duplicate payslip, missing bank details, expired contract) that runs before Validate is allowed to succeed."
  → commit: `[BE-08.2] Validate + mark-paid + warnings`

### BE-09 · PDF & Email Delivery
- **F1:** "Build `services/pdf.service.ts` using Puppeteer to render a payslip HTML template to PDF, exposed as `GET /payslips/:id/pdf`."
  → commit: `[BE-09.1] Payslip PDF generation`
- **F2:** "Build `services/email.service.ts` (Nodemailer) and `POST /payruns/:id/send` that emails each employee their PDF, logging delivery status per payslip."
  → commit: `[BE-09.2] Bulk email delivery`

### BE-10 · Dashboard APIs, ML Client & Seeding
- **F1:** "Build dashboard aggregation endpoints: KPI totals, salary-by-department, monthly trend, attendance stats, leave stats — all live-queried, matching the frontend's expected shapes in the OpenAPI contract."
  → commit: `[BE-10.1] Dashboard aggregation APIs`
- **F2:** "Build `services/ml-client.service.ts` — a thin HTTP client to the ML microservice per `shared/api-contracts/ml-endpoints.md`, with a fallback (return empty/neutral data) if the ML service is unreachable, so the core app never breaks."
  → commit: `[BE-10.2] ML client service with fallback`
- **F3:** "Write `db/seed.ts`: 20+ employees across 3+ departments, 6 months of contracts/attendance/leave history, and at least 3 completed payruns."
  → commit: `[BE-10.3] Full demo data seeding`

---

## Your Integration Sync Points

- BE-02 is the most time-sensitive fraction on the whole team — Person 1 and Person 3 both wait on `shared/api-contracts/openapi.yaml` to start building against typed mocks. Land it early.
- As you finish BE-04 → BE-08, ping Person 1 so they can flip their modules from mock data to real calls.
- Watch `shared/api-contracts/ml-endpoints.md` (Person 3 owns it) — build BE-10.2's `ml-client.service.ts` against it as soon as it has stub responses, don't wait for real ML models.
- Watch `shared/api-contracts/CHANGE-REQUESTS.md` regularly — you're the one who actions schema/API change requests from the other two.

## PR Checklist (before merging a module into `dev`)
- [ ] Only files inside `backend/` (or an approved `shared/` change) are touched
- [ ] Each fraction is its own commit with the `[BE-NN.N]` prefix
- [ ] No mega-commit — split any giant AI-generated diff into logical commits before pushing
- [ ] Branch rebased on latest `dev`
- [ ] Any contract change is reflected in `shared/api-contracts/openapi.yaml` and the corresponding `CHANGE-REQUESTS.md` entry is cleared
