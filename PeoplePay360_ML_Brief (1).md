...
---
## Part 2 — Person 3 (ML/Intelligent Layer): Your Module Breakdown & Prompts

You own `ml-service/`. Never ask the AI generator to write files outside it — for backend/frontend needs, they'll come to you via the shared contract handshake in §8 above. Work through the modules in order; within a module, run one fraction at a time, review the diff, commit, then move to the next fraction.

### ML-01 · Service Scaffold
- **F1:** "Scaffold a FastAPI app in `ml-service/app/` with a `/health` endpoint, pydantic base schemas, and a `routers/` folder structure matching the planned features (warnings, anomalies, forecast, risk, nlp, predictions). Add `requirements.txt`."
  → commit: `[ML-01.1] FastAPI scaffold + health route`
- **F2:** "Document all planned endpoint request/response shapes in `shared/api-contracts/ml-endpoints.md` so Person 2 can build the client against it before models exist (return hardcoded example JSON from each endpoint for now)."
  → commit: `[ML-01.2] ML endpoint contract + stub responses`

### ML-02 · Smart Payroll Warning Engine
- **F1:** "Implement `POST /warnings/payrun`: duplicate payslip detection, salary-spike detection (>25% vs 3-month rolling avg), missing bank/email checks, expired contract checks. Return a structured list of `{type, severity, affectedEmployeeId}`."
  → commit: `[ML-02.1] Payroll warning engine`
- **F2:** "Add a 0–100 readiness score aggregating warning severities, with a Green/Amber/Red label."
  → commit: `[ML-02.2] Payrun readiness score`

### ML-03 · Attendance Anomaly Detection
- **F1:** "Implement `POST /anomalies/attendance`: Z-score on check-in delta from scheduled start time per employee, flag bulk-identical-timestamp patterns."
  → commit: `[ML-03.1] Attendance anomaly detection`
- **F2:** "Add per-employee and per-department Attendance Health Score (0–100) combining late/absent/missing-checkout/overtime rates."
  → commit: `[ML-03.2] Attendance health score`

### ML-04 · Leave Pattern Prediction
- **F1:** "Implement `GET /predictions/leave?dept=X&month=Y` using a simple time-series model (Prophet or seasonal averaging if data is sparse) over historical leave requests, returning expected leave days."
  → commit: `[ML-04.1] Leave pattern prediction`

### ML-05 · Salary Cost Forecasting
- **F1:** "Implement `POST /forecast/salary` — regression over historical payrun totals + current headcount/contracts, returning next-month forecast per department with a confidence range."
  → commit: `[ML-05.1] Salary cost forecasting`

### ML-06 · Attrition Risk Scoring
- **F1:** "Implement `POST /risk/attrition` — a weighted logistic-regression-style scorer over attendance irregularity, leave frequency, contract type, and salary growth, returning Low/Medium/High per employee."
  → commit: `[ML-06.1] Attrition risk scoring`

### ML-07 · NLP Query Assistant
- **F1:** "Implement `POST /nlp/query` — accept a plain-English question, use an LLM API call to translate it into a safe parameterized query against a read-only DB view/replica, execute it, and return a plain-English answer plus the raw data used."
  → commit: `[ML-07.1] NLP query assistant`

### ML-08 · Training Data, Seeding & Integration Polish
- **F1:** "Write `seed_synthetic_data.py` to generate realistic training data (seeded consistently with the backend's seed script) and fit all statistical models at service startup, storing them in-memory."
  → commit: `[ML-08.1] Synthetic training data + model fitting`
- **F2:** "Add error handling, request validation, and consistent error response shape across all routers; verify `/health` reports model-load status."
  → commit: `[ML-08.2] Error handling + health status polish`

---

## Your Integration Sync Points

- Land ML-01.2 (stub contract in `shared/api-contracts/ml-endpoints.md`) as early as possible — Person 2 builds `ml-client.service.ts` against your stubs and Person 1 builds dashboard widgets against them too, so neither of them waits on your real models.
- As you finish ML-02 → ML-07, ping Person 2 so the backend can swap stub responses for real calls to your service, one endpoint at a time.
- Keep your synthetic seed data (ML-08.1) aligned with Person 2's `db/seed.ts` — same employee/department names and counts, so demo numbers actually make sense across both services.

## PR Checklist (before merging a module into `dev`)
- [ ] Only files inside `ml-service/` (or an approved `shared/` change) are touched
- [ ] Each fraction is its own commit with the `[ML-NN.N]` prefix
- [ ] No mega-commit — split any giant AI-generated diff into logical commits before pushing
- [ ] Branch rebased on latest `dev`
- [ ] Any contract change is reflected in `shared/api-contracts/ml-endpoints.md` and the corresponding `CHANGE-REQUESTS.md` entry is cleared
